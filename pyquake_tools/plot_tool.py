...
class Ptool:
    ...
    # Minimum effective normal stress to be plotted (LOG10 SCALE)
    normal_min = 10
    # Maximum effective normal stress  to be plotted (LOG10 SCALE)
    normal_max = 100
    ...
    def read_statefile(self):
        ...
        vmax = pd.read_csv(
            ...
            names=['Iteration', 'dt', 'slipv1',
                   'slipv2', 'time(s)', 'time(h)'],
            engine='python',
    ...
    def plot_timeseries2(self):
        ...
        fig, (ax, ax1, ax2) = plt.subplots(
            3, 1, figsize=(10, 12), sharex=True, clear=True)
    ...
    def animation3D(self):
        ...
        tri_cells = mesh.extract_cells(
            np.where(mesh.celltypes == pv.CellType.TRIANGLE)[0])
        # skip the first '3' entry
        triangles = tri_cells.cells.reshape(-1, 4)[:, 1:4]
        ...
        if self.var == 'V':
            ...
            label = '$\\Omega$ [-]'
            cmap = 'seismic'
            data = mesh.cell_data['Slipv[m/s]'] * \
                mesh.cell_data['state'] / mesh.cell_data['dc']
            surf.set_array(data)
    ...
    def extract_slip_info(self):
        ...
        for i in range(Nevents-1):
            ...
            step_min = self.steps[iter_indices].min()
            # Finish index of the event
            step_max = self.steps[iter_indices].max()
            ...
            try:
                ...
                for ii in range(N_iter):
                    ...
                    if ii == 0:
                        # Nucleation Point
                        Nuc = ((X_min1+X_max1)*0.5, (Y_min1+Y_max1)
                               * 0.5, (Z_min1+Z_max1)*0.5)
    ...
    def event_plot(self):
        ...
        ax.plot((self.x_mean + x1) * 1e-3, t1 +
                time_m.min()+5, ls='--', color='k')
        ax.plot((self.x_mean - x1) * 1e-3, t1 +
                time_m.min()+5, ls='--', color='k')

...
class QDsim:
    ...
    def calc_corefunc(self):
        ...
        if(self.Para0['H-matrix']=='True'):
            ...
            # gr=hm.transgr(self.nodelst,self.elelst,self.eleVec,self.xg)
            ...
            #Hmat.createHmatrix(self.xg,self.nodelst,self.elelst,self.eleVec,self.mu,self.lambda_,self.halfspace_jud)
    ...
    def derivative(self,Tno,Tt1o,Tt2o,state):
        ...
        #print("Ax time taken: %.6f seconds\n"%timetake)   
    ...
    def derivative_gpu(self,Tno,Tt1o,Tt2o,state):
        ...
        dtau1dt=(-self.AdotV1+self.shear_loading_gpu-self.mu/(2.0*self.Cs)*(dV1dsigma*self.dsigmadt+dV1dstate*dstatedt))/(1.0+self.mu/(2.0*self.Cs)*dV1dtau)
        dtau2dt=(-self.AdotV2+self.shear_loading_gpu-self.mu/(2.0*self.Cs)*(dV2dsigma*self.dsigmadt+dV2dstate*dstatedt))/(1.0+self.mu/(2.0*self.Cs)*dV2dtau)
        #dtau2dt=(AdotV2+self.shear_loading-self.mu/(2.0*self.Cs)*np.sin(self.rake)*(dVdsigma*dsigmadt+dVdstate*dstatedt))/(1.0+self.mu/(2.0*self.Cs)*dVdtau)
        #dVdt=dVdtao*dtaodt+dVdsigma*dsigmadt+dVdstate*dstatedt
    ...
    def simu_forwardGPU(self,dttry):
        ...
        self.fric_gpu=self.Tt_gpu/self.Tno_gpu
